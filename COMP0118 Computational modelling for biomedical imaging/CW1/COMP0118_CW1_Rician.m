...
res_plot = zeros(nb_of_retries, nb_of_steps);
%h=optimset('MaxFunEvals',20000,'Algorithm','quasi-newton','TolX',1e-6,'TolFun',1e-6, 'Display', 'Iter');
h=optimset('MaxFunEvals',20000,'Algorithm','trust-region','TolX',1e-9,'TolFun',1e-9,'DerivativeCheck','on');
%h=optimset('MaxFunEvals',20000,'Algorithm','interior-point','TolX',1e-9,'TolFun',1e-9,'DerivativeCheck','on');
...
for ll=1:nb_of_retries
    for kk=1:nb_of_steps
        ...
        [ouaiscquoi,RESNORM,~,~]=BallStickSSDOptim2GRAD(startx, Avox, bvals, qhat, h);
        res_plot(ll, kk) = RESNORM;
    end
end
...
x_1 = [3300 1.0e-03 2.5e-01 1.0 1.0];
...
for kk=2:mcmc_steps
    ...
    x_2 = x_1 + [normrnd(0, 0.25) normrnd(0, 0.25) normrnd(0.5, 0.125) normrnd(0, 0.125) normrnd(0, 0.125)]; % Our perturbation
    like_x_2 = BallStickSSD(x_2, Avox, bvals, qhat);

    ratio_like = like_x_1/like_x_2;
    ratio_like_arr(kk) = ratio_like;

    if(ratio_like > unifrnd(0, 10))
        x_1 = x_2;
        like_x_1 = like_x_2;
...
disp('Acceptance rate: '+string(acceptance_rate/mcmc_steps));
...
function [parameter_hat,RESNORM,EXITFLAG,OUTPUT] = BallStickSSDOptim(x, Avox, bvals, qhat, h)
    ...
    function sumRes = BallStickSSD(x)
        ...
        %sumRes = sum((Avox - S').^2);
        sumRes = sum((Avox - S').^2) - 0.5*log(sum(Avox - S'));
    end
end
